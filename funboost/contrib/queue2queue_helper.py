...
def queue2queue(source_queue_name: str, source_broker_kind: str,
...
def _queue2queue_for_multi_process(source_queue_name: str, source_broker_kind: str,
                                                         target_queue_name: str, target_broker_kind: str,
                                                         log_level: int = logging.DEBUG,
                                                         ):
    queue2queue(source_queue_name, source_broker_kind, target_queue_name, target_broker_kind, log_level, False)
    while 1:
        time.sleep(3600)
...
def multi_prcocess_queue2queue(source_target_list: typing.List[typing.List],
    ...
    for (source_queue_name, source_broker_kind, target_queue_name, target_broker_kind) in source_target_list:
        for i in range(n):
            Process(target=_queue2queue_for_multi_process,
...
if __name__ == '__main__':
    # 一次转移一个队列，使用单进程
    queue2queue('test_queue77h3_dlx', BrokerEnum.REDIS_PRIORITY,
