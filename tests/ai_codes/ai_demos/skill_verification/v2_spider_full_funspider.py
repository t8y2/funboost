"""验证 funboost-spider-crawling SKILL — 完整 funspider 示例（结构/导入/参数，不发网络请求）"""
import os
import re
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_spider_full_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_spider_full_std_{_ts}"
...
NEWS_GROUP = "v2_news_crawler_full"
...
SQLITE_ENGINE = create_engine(f"sqlite:///v2_spider_full_{_ts}.db")
...
@boost(NewsCrawlerParams(queue_name="v2_full_news_list", qps=2))
def crawl_list(page: int):
    return page


@boost(NewsCrawlerParams(
    queue_name="v2_full_news_detail", qps=5, max_retry_times=5,
...
@boost(NewsCrawlerParams(
    queue_name="v2_full_news_comments", qps=10,
