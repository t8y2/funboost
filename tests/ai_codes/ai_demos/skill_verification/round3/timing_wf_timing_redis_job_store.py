...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"timing_wf_timing_redis_job_store_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"timing_wf_timing_redis_job_store_std_{_ts}"

from funboost import boost, BoosterParams, BrokerEnum, ApsJobAdder


@boost(BoosterParams(
    queue_name="r3_cleanup_cache_task",
...
if __name__ == "__main__":
    print("[START] timing_wf_timing_redis_job_store (memory job_store)")
    ...
    time.sleep(20)
    print("[DONE] timing_wf_timing_redis_job_store")
    os._exit(66)
