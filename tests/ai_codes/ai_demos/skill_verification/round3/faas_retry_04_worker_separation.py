"""round3 验证: funboost-faas-deploy — 消费端 Worker 分离示例"""
import os
import time

_ts = int(time.time())
os.environ["LOG_PATH"] = r"D:\pythonlogs\ai_console_outs"
os.environ["PRINT_WRTIE_FILE_NAME"] = f"faas_retry_04_worker_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"faas_retry_04_worker_std_{_ts}"
...
try:
    ...
    @boost(BoosterParams(
        queue_name="r3_faas_process_order",
        broker_kind=BrokerEnum.MEMORY_QUEUE,
        concurrent_num=2,
    ))
    ...
    report(callable(enable_ctrl_c_quit_on_windows), "enable_ctrl_c_quit_on_windows 可导入")
