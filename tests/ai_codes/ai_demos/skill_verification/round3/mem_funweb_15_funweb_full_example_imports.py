...
os.environ["PRINT_WRTIE_FILE_NAME"] = f"mem_funweb_15_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"mem_funweb_15_std_{_ts}"
...
@boost(MyBoosterParams(queue_name=f"queue_test_g01t_r3_{_ts}", qps=1))
def f(x):
    time.sleep(0.01)
...
@boost(MyBoosterParams(queue_name=f"queue_test_g02t_r3_{_ts}", qps=0.5, max_retry_times=0))
def f2(x, y):
    time.sleep(0.01)
...
@boost(MyBoosterParams(
    queue_name=f"queue_test_g03t_r3_{_ts}",
    qps=0.5,
    max_retry_times=0,
