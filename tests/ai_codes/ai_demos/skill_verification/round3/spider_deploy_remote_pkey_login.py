"""round3 验证 funboost-remote-deploy SKILL — SSH 私钥登录 pkey_file_path"""
import inspect
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"r3_remote_pkey_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"r3_remote_pkey_std_{_ts}"
...
@boost(BoosterParams(queue_name="r3_my_task_pkey", broker_kind=BrokerEnum.MEMORY_QUEUE))
def my_task(x):
    return x
