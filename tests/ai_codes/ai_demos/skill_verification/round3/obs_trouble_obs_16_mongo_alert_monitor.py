"""验证 round3 / funboost-observability SKILL §5 MongoAlertMonitor"""
import os
import time
import threading
...
os.environ["PRINT_WRTIE_FILE_NAME"] = f"obs_trouble_obs_16_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"obs_trouble_obs_16_std_{_ts}"
...
@boost(BoosterParams(
    queue_name=f"obs_trouble_obs_monitor_task_{_ts}",
...
@boost(BoosterParams(
    queue_name=f"obs_trouble_obs_monitor_a_{_ts}",
    broker_kind=BrokerEnum.MEMORY_QUEUE,
    concurrent_num=2,
    function_result_status_persistance_conf=FunctionResultStatusPersistanceConfig(
        is_save_status=True,
