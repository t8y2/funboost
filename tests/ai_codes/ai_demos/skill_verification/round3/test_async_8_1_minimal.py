"""验证 SKILL: funboost-async-programming §8.1 无 Redis 依赖最小完整示例"""
import asyncio
import os
import time
...
os.environ["PRINT_WRTIE_FILE_NAME"] = f"test_async_8_1_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"test_async_8_1_std_{_ts}"
...
@boost(
    BoosterParams(
        queue_name=f"skill_async_demo_q_{_ts}",
...
@boost(
    BoosterParams(
        queue_name=f"skill_sync_demo_q_{_ts}",
        broker_kind=BrokerEnum.MEMORY_QUEUE,
    )
)
def sync_task(n: int):
