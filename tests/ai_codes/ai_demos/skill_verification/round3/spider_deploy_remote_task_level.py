"""round3 验证 funboost-remote-deploy SKILL — 函数级粒度部署（.fabric_deploy 方法）"""
import inspect
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"r3_remote_task_level_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"r3_remote_task_level_std_{_ts}"
...
@boost(BoosterParams(queue_name="r3_queue_a", broker_kind=BrokerEnum.MEMORY_QUEUE))
def task_a(x):
    return x * 2


@boost(BoosterParams(queue_name="r3_queue_b", broker_kind=BrokerEnum.MEMORY_QUEUE))
def task_b(x):
    return x + 1
