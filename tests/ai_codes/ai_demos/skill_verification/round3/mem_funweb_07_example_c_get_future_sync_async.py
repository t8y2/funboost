...
os.environ["PRINT_WRTIE_FILE_NAME"] = f"mem_funweb_07_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"mem_funweb_07_std_{_ts}"

from funboost import boost, BoosterParams, BrokerEnum, ConcurrentModeEnum


@boost(BoosterParams(queue_name=f"sync_q_r3_{_ts}", broker_kind=BrokerEnum.MEMORY_QUEUE, qps=2, concurrent_num=10))
def sync_task(x, y):
    time.sleep(0.5)
    return x + y
