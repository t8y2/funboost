"""验证 funboost-remote-deploy SKILL — extra_shell_str 部署前环境变量"""
import inspect
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_remote_shell_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_remote_shell_std_{_ts}"
...
@boost(BoosterParams(queue_name="v2_my_task_shell", broker_kind=BrokerEnum.MEMORY_QUEUE))
def my_task(x):
    return x
