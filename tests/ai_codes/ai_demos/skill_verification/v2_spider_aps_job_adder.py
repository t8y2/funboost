"""验证 funboost-spider-crawling SKILL — ApsJobAdder 定时爬取种子"""
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_spider_aps_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_spider_aps_std_{_ts}"

PASS = True
CRAWLER_GROUP = "v2_seed_crawler"
...
@boost(BoosterParams(queue_name="v2_seed_scheduler", broker_kind=BrokerEnum.MEMORY_QUEUE, booster_group=CRAWLER_GROUP))
def push_daily_seeds():
    return list(range(1, 11))


@boost(BoosterParams(queue_name="v2_crawl_list_seed", broker_kind=BrokerEnum.MEMORY_QUEUE))
