...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_workflow_chain_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_workflow_chain_std_{_ts}"
...
@boost(WfParams(queue_name="v2_wf_chain_download"))
def download(url: str):
    print(f"下载 {url}")
    return f"/tmp/{url.split('/')[-1]}"


@boost(WfParams(queue_name="v2_wf_chain_process"))
