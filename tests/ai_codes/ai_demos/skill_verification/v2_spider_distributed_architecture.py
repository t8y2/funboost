"""验证 funboost-spider-crawling SKILL — 分布式爬虫架构（BoosterParams 继承 + consume_group）"""
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_spider_dist_arch_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_spider_dist_arch_std_{_ts}"
...
@boost(NewsCrawlerParams(queue_name="v2_news_list", qps=2, do_task_filtering=False))
def crawl_list(page: int):
    return page


@boost(NewsCrawlerParams(queue_name="v2_news_detail", qps=5, max_retry_times=5))
