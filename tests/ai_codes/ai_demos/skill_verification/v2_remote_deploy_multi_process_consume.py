"""验证 funboost-remote-deploy SKILL — 本地 multi_process_consume 对照"""
import inspect
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_remote_mp_consume_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_remote_mp_consume_std_{_ts}"
...
@boost(BoosterParams(queue_name="v2_my_task_mp", broker_kind=BrokerEnum.MEMORY_QUEUE))
def my_task(x):
    return x * 2
