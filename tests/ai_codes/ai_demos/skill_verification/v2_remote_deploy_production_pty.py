"""验证 funboost-remote-deploy SKILL — 生产常驻 invoke_runner_kwargs（pty=False）"""
import inspect
import os
import sys
import time
...
_ts = int(time.time())
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_remote_pty_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_remote_pty_std_{_ts}"
...
@boost(BoosterParams(queue_name="v2_my_task_pty", broker_kind=BrokerEnum.MEMORY_QUEUE))
def my_task(x):
    return x
