...
os.environ["PRINT_WRTIE_FILE_NAME"] = f"v2_mixin_06_{_ts}"
os.environ["SYS_STD_FILE_NAME"] = f"v2_mixin_06_std_{_ts}"
...
@boost(BoosterParams(
    queue_name=f"async_aware_v2_{_ts}",
...
if __name__ == "__main__":
    ...
    print("[START] v2_mixin_06_async_aware")
    ...
    asyncio.run(_main())
    # executor 路径会调用两次 _save_to_db（sync + async 各一次路径）
    if len(DB_SAVES) >= 1:
