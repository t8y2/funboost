"""
Celery 性能 Benchmark V2 —— 单线程模式（纯 CPU 任务）
消费 1 万条消息，每 1000 条打印时间
"""
import os
import time
import sys
...
os.environ['PRINT_WRTIE_FILE_NAME'] = 'benchmark_celery_v3_print.print'
os.environ['SYS_STD_FILE_NAME'] = 'benchmark_celery_v3_std.std'
...
@celery_app.task(bind=True, name='bench_task_v3', max_retries=0)
def bench_task(self: CeleryTask, msg_id: int):
    with lock:
        ...
        if count % BATCH_SIZE == 0:
            ...
            print(f'[CeleryV2] 已处理 {count}/{TOTAL_MSGS} 条, 耗时 {elapsed:.2f}秒, {rate:.0f} msg/s')


if __name__ == '__main__':
    # 预热 + 清理
    print('[CeleryV2] 预热发布连接...')
    bench_task.delay(msg_id=0)
    time.sleep(0.5)
    import redis
    rr = redis.Redis(host='127.0.0.1', port=6379, db=12)
    rr.delete('bench_task_v3')

    # ===== 发布 =====
    print(f'[CeleryV2] 开始发布 {TOTAL_MSGS} 条消息到 Redis...')
    t0 = time.time()
    ...
    print(f'[CeleryV2] 发布完成，耗时 {publish_time:.2f}秒, 平均 {TOTAL_MSGS/publish_time:.0f} msg/s')
    print()

    # ===== 消费（单线程 solo 模式） =====
    print(f'[CeleryV2] 启动 Worker(单线程) 开始消费...')
    worker_thread = threading.Thread(
        target=celery_app.worker_main,
        args=(['worker', '--pool=solo', '-n', 'worker_bm@%h',
    ...
    print('[CeleryV2] Worker 已启动...')

    sleep_time = 120  # 单线程很慢，给 120 秒
    print(f'\n[CeleryV2] 等待 {sleep_time} 秒后自动退出...')
